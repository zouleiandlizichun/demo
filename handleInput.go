...
func HandleInput() ([]byte, error) {
	scanner := bufio.NewScanner(os.Stdin)

	fmt.Println("现在功能是遥测,请输入装置地址（例如 16）：")
	scanner.Scan()
	deviceAddressstr := scanner.Text()

	fmt.Println("现在功能是遥测,请输入功能码（例如  03）：")
	scanner.Scan()
	functionCodestr := scanner.Text()

	fmt.Println("现在功能是遥测,请输入起始地址（例如 46）：")
	scanner.Scan()
	startingAddressStr := scanner.Text()

	fmt.Println("现在功能是遥测,请输入寄存器数量（例如 16）：")
	scanner.Scan()
	...
	dataWithCRC := append(data, crc...)

	//for _, value := range dataWithCRC {
	//
	//	fmt.Printf("%02x", value)
	//}

	return dataWithCRC, nil
